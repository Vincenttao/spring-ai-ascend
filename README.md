...
> Enterprise agent platform on Spring AI 2.0.0-M5 + Spring Boot 4.0.5 + Java 21 — as of v2.0.0-rc6 (2026-05-18 rc5 post-response review response on rc5).
...
- Latest release: [docs/releases/2026-05-18-l0-rc6-post-response.en.md](docs/releases/2026-05-18-l0-rc6-post-response.en.md) (v2.0.0-rc6; additive uplift on v2.0.0-rc5 — rc5 NOT retracted). Prior waves: [docs/releases/2026-05-18-l0-rc4-cross-constraint-response.en.md](docs/releases/2026-05-18-l0-rc4-cross-constraint-response.en.md) (v2.0.0-rc5), [docs/releases/2026-05-18-beyond-sdd-review-response.en.md](docs/releases/2026-05-18-beyond-sdd-review-response.en.md).
- Per-capability shipped/deferred ledger: [docs/governance/architecture-status.yaml](docs/governance/architecture-status.yaml)
- Architecture baseline (v2.0.0-rc6 — 2026-05-18 rc5 post-response review response): 65 §4 constraints · 80 ADRs · 70 active gate rules · 138 self-tests · 41 active engineering rules · 13 Layer-0 governing principles · 100 enforcer rows · 371 Maven tests GREEN · 335 architecture-graph nodes · 463 architecture-graph edges. Canonical structured baseline lives in [`docs/governance/architecture-status.yaml#architecture_sync_gate.baseline_metrics`](docs/governance/architecture-status.yaml) (single source of truth; enforced by gate Rule 82 — strengthened this wave with a numeric-agreement check so adjacent counts that disagree with the baseline now fail the gate). rc6 wave summary: (a) `agent-service/ARCHITECTURE.md` engine + S2C sections rewritten to consume from `agent-execution-engine` + `agent-runtime-core` post-ADR-0079 — present-tense future-work prose deleted; dead `docs/STATE.md` link replaced (P0-1 closure); (b) Rule 82 strengthened — numeric-drift check on entrypoint count phrases, exemption via `historical` / `rc[N] baseline` / `pre-rc[N]` / `previous` / `deprecated` / `superseded` markers (P1-1 closure); (c) `ResilienceContract` + value types (`ResiliencePolicy`, `SkillResolution`, `SuspendReason`, `SkillCapacityRegistry`) moved to `ascend.springai.service.runtime.resilience.spi.*` per ADR-0080 — implementations stay in `runtime.resilience.*`; `agent-service/module-metadata.yaml#spi_packages` + `docs/dfx/agent-service.yaml#spi_packages` + `contract-catalog.md` aligned (P1-2 closure); (d) `plan-projection.v1.yaml` comment distinguishes runtime enforcement (deferred to W2) from Rule 83 catalog-registration enforcement (active at rc5) (P2-1 closure). 2 new prevention gate rules: 84 (`active_module_architecture_path_truth`) · 85 (`catalog_spi_row_matches_module_spi_metadata`) — enforcers E117-E118. 1 new ADR: ADR-0080 (ResilienceContract `.spi` package alignment).
