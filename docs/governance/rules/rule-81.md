...
The 2026-05-18 rc4 cross-constraint architecture review (finding P0-2 in `docs/reviews/2026-05-18-l0-rc4-cross-constraint-architecture-review.en.md`) discovered that the textual sweep was incomplete:
...
A contributor following the root L0 architecture would treat `agent-execution-engine` as an empty workspace while the reactor already built it as the owner of the engine surface. This undermined Rule 31 (Independent Module Evolution) and Rule 33 (Layered 4+1 Discipline) because module identity, module architecture, and physical code stopped agreeing.

Rule 81 prevents the re-occurrence: a module can carry a `skeleton` status in its L1 ARCHITECTURE.md only when its production Java tree literally contains no production types beyond `package-info.java` (or ADR-waived placeholder stubs).
...
The 30-line window matches the convention used by Rule 75 for module-metadata `spi_packages` placeholder entries (`# placeholder; ADR-NNNN ...`). The two rules share the same waiver shape so a single ADR can license both a metadata placeholder and a code stub.
...
- Rule 31 (Independent Module Evolution) — Rule 81 protects Rule 31's invariant that every module has a coherent identity (metadata + ARCHITECTURE.md + actual Java tree).
- Rule 32 (SPI + DFX + TCK Co-Design) — placeholder SPI stubs survive Rule 32 only via the ADR waiver Rule 81 enforces.
- Rule 33 (Layered 4+1 Discipline) — module-level L1 architecture status is one of the artefacts Rule 33 freezes; Rule 81 guards the truthfulness of the status token.
- Rule 75 (SPI Packages Populated) — companion rule for the metadata side; placeholder-with-ADR shape is shared.
...
- `docs/reviews/2026-05-18-l0-rc4-cross-constraint-architecture-review.en.md` finding P0-2 — origin.
- `docs/reviews/2026-05-18-l0-rc4-cross-constraint-architecture-review-response.en.md` — response document recording the wave.
